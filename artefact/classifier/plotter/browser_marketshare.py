...
plot = sns.barplot(x='Browsers', y='Percentage',data=plotdata, palette=colors,)
locs, labels = plt.xticks()
plot.set_xticklabels(labels=labels)
for p in plot.patches:
    plot.annotate("%.2f" % p.get_height(), (p.get_x() + p.get_width() / 2., p.get_height()),ha='center', va='center', fontsize=11, color='gray', xytext=(0, 20),textcoords='offset points')
