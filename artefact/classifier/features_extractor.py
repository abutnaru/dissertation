...
features and saves the records in a CSV file. Selected features are: 
"""
import csv
import re
import subprocess
...
def has_domain(target_path, N):
    with open("data/processed_sets/benign_1M.csv") as benign_list:
        index = 0
        reader = csv.reader(benign_list)
        for row in reader:
            index += 1
...
def extract(url, label=-1):
    ...
    if path_occs == 0:
        path_occs = (
            1
            if has_domain(parsed_url.path, 2500) or has_domain(parsed_url.query, 2500)
            else 0
        )
    ...
    dom_distance, subdom_distance = min_hamming(url, 25_000)
